...
function i18nTitle() {
    return window.konsultasiI18n?.title || (isIndonesian() ? "LAPORAN PENGAJUAN KONSULTASI KEARSIPAN" : "ARCHIVAL CONSULTATION REQUEST REPORT");
}
...
function buildTableHTML(dt) {
    ...
    var colWidths = [40, 180, 150, 200, 110];
...
function exportToWord(dt) {
    ...
    var fname = isIndonesian()
        ? "Laporan-Pengajuan-Konsultasi-DABB.doc"
        : "Archival-Consultation-Report-DABB.doc";
    downloadBlob(new Blob(["\uFEFF" + html], { type: "application/msword" }), fname);
}

function exportToPDF(dt) {
    var data = readTableData(dt);
    var colWidths = [30, 160, 140, 220, 110];
    ...
    var fname = isIndonesian()
        ? "Laporan-Pengajuan-Konsultasi-DABB.pdf"
        : "Archival-Consultation-Report-DABB.pdf";
...
function exportToCSV(dt) {
    ...
    var fname = isIndonesian()
        ? "Laporan-Pengajuan-Konsultasi-DABB.csv"
        : "Archival-Consultation-Report-DABB.csv";
...
function exportToExcel(dt) {
    ...
    var fname = isId
        ? "Laporan-Pengajuan-Konsultasi-DABB.xls"
        : "Archival-Consultation-Report-DABB.xls";
    ...
    loadLogoBase64(function(logoBase64) {
        ...
        var htmlContent =
            ...
            '<!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>Konsultasi</x:Name><x:WorksheetOptions><x:Print><x:ValidPrinterInfo/></x:Print></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]-->' +
...
$(function () {
    ...
    if (!$("#tableKonsultasi").length) return;

    var i18n = window.konsultasiI18n || {};
