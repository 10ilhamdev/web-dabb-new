function pengelolaanForm(initialType = 'penyusutan', initialExtraData = null) {
    return {
        type: initialType,
        files: [],
        ...
        // Penyimpanan
        prinsip_title: initialExtraData?.prinsip_title ?? "Prinsip Penyimpanan Arsip Statis",
        prinsip_desc: initialExtraData?.prinsip_desc ?? (initialExtraData?.prinsip_penyimpanan ?? "Penyimpanan arsip statis dilaksanakan berdasarkan prinsip:"),
        prinsip_list: initialExtraData?.hasOwnProperty('prinsip_list') ? initialExtraData.prinsip_list : [
        ...
        sistem_title: initialExtraData?.sistem_title ?? "Sistem Penyimpanan",
        sistem_penyimpanan: initialExtraData?.hasOwnProperty('sistem_penyimpanan') ? initialExtraData.sistem_penyimpanan : [
        ...
        fasilitas_title: initialExtraData?.fasilitas_title ?? "Fasilitas Penyimpanan",
        fasilitas_list: initialExtraData?.hasOwnProperty('fasilitas_list') ? initialExtraData.fasilitas_list : [
        ...
        ruang_title: initialExtraData?.ruang_title ?? "Ruang Penyimpanan",
        ruang_list: initialExtraData?.hasOwnProperty('ruang_list') ? initialExtraData.ruang_list : [
        ...
        // Preservasi
        preservasi_title: initialExtraData?.preservasi_title ?? "Kegiatan Preservasi",
        preservasi_list: initialExtraData?.hasOwnProperty('preservasi_list') ? initialExtraData.preservasi_list : [
        ...
        restorasi_title: initialExtraData?.restorasi_title ?? "RESTORASI ARSIP",
        restorasi_desc: initialExtraData?.restorasi_desc ?? "Restorasi arsip adalah tindakan khusus untuk memperbaiki dan memulihkan fisik arsip yang mengalami kerusakan akibat faktor usia, bencana, jamur, atau penanganan yang salah, agar kembali kukuh dan informasinya dapat terselamatkan.",
        restorasi_list: initialExtraData?.hasOwnProperty('restorasi_list') ? initialExtraData.restorasi_list : [
        ...
        // Pengolahan
        pengolahan_title: initialExtraData?.pengolahan_title ?? "Tahapan Pengolahan",
        pengolahan_title_en: initialExtraData?.pengolahan_title_en ?? "Processing Stages",
        pengolahan_list: initialExtraData?.hasOwnProperty('pengolahan_list') ? initialExtraData.pengolahan_list : [
        ...
        // Pemanfaatan
        mekanisme_title: initialExtraData?.mekanisme_title ?? "Mekanisme Pemanfaatan dan Akses Arsip",
        mekanisme_desc: initialExtraData?.mekanisme_desc ?? "Masyarakat dapat mengakses dan memanfaatkan khazanah arsip statis yang tersimpan di ANRI untuk berbagai kepentingan, seperti penelitian akademis, penulisan sejarah, pembuktian hukum, dan edukasi masyarakat.",
        pemanfaatan_quote: initialExtraData?.pemanfaatan_quote ?? "Berdasarkan Undang-Undang Nomor 43 Tahun 2009 tentang Kearsipan, arsip statis yang dikelola oleh lembaga kearsipan terbuka untuk umum bagi kepentingan penelitian, sejarah, dan ilmu pengetahuan, kecuali arsip yang dinyatakan tertutup berdasarkan ketentuan peraturan perundang-undangan.",
        akses_title: initialExtraData?.akses_title ?? "Layanan Akses & Pemanfaatan",
        akses_title_en: initialExtraData?.akses_title_en ?? "Access & Utilization Services",
        akses_list: initialExtraData?.hasOwnProperty('akses_list') ? initialExtraData.akses_list : [
            ...
            { title: "Akses Arsip Terbatas / Tertutup", desc: "Arsip yang memerlukan izin khusus atau masa retensi kerahasiaan tertentu sebelum dapat dibuka untuk umum.", icon: "lock" },
        ...
        // Penjangkauan
        kegiatan_title: initialExtraData?.kegiatan_title ?? "Program & Kegiatan Penjangkauan",
        kegiatan_list: initialExtraData?.hasOwnProperty('kegiatan_list') ? initialExtraData.kegiatan_list : [
        ...
        // Akuisisi
        tahapan_title: initialExtraData?.tahapan_title ?? "Tahapan & Prosedur Akuisisi",
        tahapan_list: initialExtraData?.hasOwnProperty('tahapan_list') ? initialExtraData.tahapan_list : [
