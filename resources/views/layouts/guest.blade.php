...
<head>
    ...
    <style>
        ...
        @keyframes cursor-spin-animation {
            from { transform: translate(-50%, -50%) rotate(0deg); }
            to { transform: translate(-50%, -50%) rotate(360deg); }
        }

        /* Interactive Futuristic Hover Transitions for page content */
        h1, h2, h3, h4, h5, h6 {
            transition: text-shadow 0.3s cubic-bezier(0.25, 1, 0.5, 1), color 0.3s cubic-bezier(0.25, 1, 0.5, 1), transform 0.3s cubic-bezier(0.25, 1, 0.5, 1);
        }
        ...
        .card, .bg-white.shadow-md {
            transition: transform 0.35s cubic-bezier(0.25, 1, 0.5, 1), box-shadow 0.35s cubic-bezier(0.25, 1, 0.5, 1), border-color 0.35s;
        }
        .card:hover, .bg-white.shadow-md:hover {
            transform: translateY(-6px) scale(1.008);
            box-shadow: 0 15px 35px -5px rgba(0, 242, 254, 0.18), 0 0 15px rgba(0, 242, 254, 0.1);
            border-color: rgba(0, 242, 254, 0.35);
        }
        li, tr {
            transition: transform 0.3s cubic-bezier(0.25, 1, 0.5, 1), background-color 0.3s cubic-bezier(0.25, 1, 0.5, 1);
        }
        li:not(nav li):hover, tr:not(thead tr):hover {
        ...
        .reading-progress-bar {
            ...
            box-shadow: 0 0 10px rgba(0, 242, 254, 0.8), 0 0 20px rgba(255, 0, 127, 0.6);
            z-index: 10000000;
...
<body class="@yield('body-class', 'font-sans text-gray-900 antialiased flex flex-col min-h-screen')">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // --- Scroll Reveal Logic ---
            // Automatically find elements that should animate into view on guest pages
            var elementsToAnimate = [];
            // Target elements: headings, paragraphs, divs/sections/cards, images, tables, carousels, spans, lists representing blocks of content
            document.querySelectorAll('h1, h2, h3, h4, h5, h6, p, .card, .profile-section-desc, .vsshow-section-desc, .richtext-guest-view, img, table, .rte-table-container, .rte-carousel-container, .bg-white.shadow-md, li, tr, span.font-medium, span.text-sm').forEach(function(el) {
                // Exclude navbar, footer, chat widget, modals, captions, or inside form controls
                if (el.closest('nav') || el.closest('footer') || el.closest('#chat-widget') || el.closest('#rte-caption-modal') || el.closest('form button') || el.closest('form label') || el.closest('thead')) {
                    return;
                }
                // Don't apply to very small/nested inline text or specific widgets
                if (el.classList.contains('reveal-on-scroll') || el.classList.contains('rte-table-toggle-btn') || el.classList.contains('rte-chunk-toggle-btn') || el.closest('.rte-table-toggle-bar') || el.closest('.rte-toc-block')) {
                    return;
                }
            ...
            elementsToAnimate.forEach(function(el) {
                ...
                // Classify animation type: scale-in for structural/media blocks, fade-up for text elements
                if (el.tagName === 'IMG' || el.classList.contains('card') || el.classList.contains('bg-white') || el.classList.contains('rte-table-container') || el.classList.contains('rte-carousel-container') || el.tagName === 'TABLE') {
            ...
            // Set up Staggered Intersection Observer to trigger reveal animation sequentially
            var revealQueue = [];
            ...
            var revealObserver = new IntersectionObserver(function(entries, observer) {
                ...
                if (revealQueue.length > 0) {
                    ...
                    revealTimeout = setTimeout(function() {
                        // Sort elements by their current viewport offset top/left to reveal them in natural reading flow
                        ...
                        revealQueue.forEach(function(el, index) {
                            setTimeout(function() {
                                el.classList.add('revealed');
                            }, index * 80); // Stagger interval (80ms)
                        });
                        revealQueue = [];
                    }, 50);
                }
                ...
                threshold: 0.02,
                rootMargin: '0px 0px -40px 0px' // starts slightly before entering
            });
