...
@section('header')
    <div class="text-[13px] text-gray-500 font-medium">
        ...
        <span class="text-[#0ea5e9]">Page Views</span>
    </div>
@endsection
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
            ...
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 shrink-0">
                ...
                <div class="inline-flex bg-white p-1.5 rounded-xl shadow-sm border border-gray-100 text-xs font-medium text-gray-600 overflow-x-auto">
                    <a href="{{ route('cms.reports.pengunjung.page_views', ['tf' => 'day']) }}" class="px-3 py-1.5 rounded-lg transition-colors whitespace-nowrap {{ $tf === 'day' ? 'bg-[#174E93] text-white font-bold' : 'hover:bg-gray-100' }}">{{ __('cms.reports.preset_today') }}</a>
                    <a href="{{ route('cms.reports.pengunjung.page_views', ['tf' => 'week']) }}" class="px-3 py-1.5 rounded-lg transition-colors whitespace-nowrap {{ $tf === 'week' ? 'bg-[#174E93] text-white font-bold' : 'hover:bg-gray-100' }}">{{ __('cms.reports.preset_week') }}</a>
                    <a href="{{ route('cms.reports.pengunjung.page_views', ['tf' => 'month']) }}" class="px-3 py-1.5 rounded-lg transition-colors whitespace-nowrap {{ $tf === 'month' ? 'bg-[#174E93] text-white font-bold' : 'hover:bg-gray-100' }}">{{ __('cms.reports.preset_month') }}</a>
                    <a href="{{ route('cms.reports.pengunjung.page_views', ['tf' => 'year']) }}" class="px-3 py-1.5 rounded-lg transition-colors whitespace-nowrap {{ $tf === 'year' ? 'bg-[#174E93] text-white font-bold' : 'hover:bg-gray-100' }}">{{ __('cms.reports.preset_year') }}</a>
                ...
                <form method="GET" action="{{ route('cms.reports.pengunjung.page_views') }}" x-show="showCustom" x-transition.opacity class="flex items-center gap-2 bg-white p-1.5 rounded-xl shadow-sm border border-gray-100 shrink-0 overflow-x-auto" x-cloak>
...
@push('scripts')
    ...
    <script>
        window.pengunjungI18n = {
            ...
            title: {!! json_encode(__('cms.reports.pengunjung_title')) !!},
            seriesName: {!! json_encode(__('cms.reports.chart_views_title')) !!},
            tooltipUnit: {!! json_encode(__('cms.reports.total_views')) !!},
        };
