                ...
                <table id="tableBooks" class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50 text-gray-600 text-sm font-medium border-b border-gray-100">
                            ...
                            <th class="px-6 py-4 w-48 text-center">Status</th>
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors group">
                                ...
                                <td class="px-6 py-4">
                                    <div class="vt-room-badge">
                                        ...
                                        @php
                                            $displayPages = (!empty($book->pdf_path) && !empty($book->total_pages)) 
                                                ? $book->total_pages 
                                                : $book->pages()->count();
                                            $displayPages = preg_replace('/(\s*halaman|\s*pages)/i', '', $displayPages);
                                        @endphp
                                        {{ $displayPages }} {{ app()->getLocale() === 'en' ? 'Pages' : 'Halaman' }}
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <form action="{{ route('cms.features.virtual_books.toggle_visibility', [$feature, $book]) }}" method="POST" class="inline">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" title="{{ $book->is_active ? 'Sembunyikan' : 'Tampilkan' }}"
                                            class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold border transition-colors
                                            {{ $book->is_active ? 'bg-green-50 text-green-600 border-green-200 hover:bg-green-100' : 'bg-gray-100 text-gray-500 border-gray-200 hover:bg-gray-200' }}">
                                            @if($book->is_active)
