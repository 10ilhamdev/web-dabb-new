                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <form action="{{ route('cms.features.toggle-visibility', $feature) }}" method="POST" class="inline">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit"
                                        class="inline-flex items-center justify-center w-8 h-8 {{ $feature->is_active ? 'bg-gray-500 hover:bg-gray-600' : 'bg-gray-400 hover:bg-gray-500' }} text-white rounded-md transition-colors"
                                        title="{{ $feature->is_active ? 'Sembunyikan' : 'Tampilkan' }}">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <!-- Edit Button -->
                                <button @click="openEditModal({{ $feature->id }}, '{{ addslashes($feature->name) }}', '{{ $feature->type }}', '{{ $feature->path ?? '' }}', {{ $feature->order }}, '{{ $feature->page_type ?? 'none' }}', '{{ $feature->parent_id ?? '' }}', {{ $feature->is_login_required ? 'true' : 'false' }})"
    ...
    <div x-show="editModal.open" x-cloak
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md z-[9999]"
            ...
            <form :action="`/cms/features/${editModal.id}`" method="POST" class="px-6 py-5 space-y-4">
                ...
                <div class="border-t border-gray-100 pt-4" x-show="editModal.type === 'link'">
                    <label class="flex items-center gap-2.5 cursor-pointer select-none">
                        <div class="relative">
                            <input type="checkbox" name="is_login_required" id="edit_is_login_required"
                                x-model="editModal.isLoginRequired"
                                value="1"
                                class="sr-only peer">
    ...
    <div x-show="addModal.open" x-cloak
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md z-[9999]"
            ...
            <form action="{{ route('cms.features.store') }}" method="POST" class="px-6 py-5 space-y-4">
                ...
                <div class="border-t border-gray-100 pt-4" x-show="addModal.type === 'link'">
                    <label class="flex items-center gap-2.5 cursor-pointer select-none">
                        <div class="relative">
                            <input type="checkbox" name="is_login_required" id="add_is_login_required"
                                x-model="addModal.isLoginRequired"
                                value="1"
                                class="sr-only peer">
                            <div class="w-10 h-5 bg-gray-200 rounded-full peer peer-checked:bg-[#174E93] transition-colors"></div>
                            <div class="absolute left-0.5 top-0.5 bg-white w-4 h-4 rounded-full transition-transform peer-checked:translate-x-5 shadow-sm"></div>
