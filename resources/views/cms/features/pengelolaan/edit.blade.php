...
@endsection
@section('breadcrumb_active', __('cms.pengelolaan.edit_button'))

@section('content')
<div class="px-4 py-6 max-w-7xl mx-auto sm:px-6 lg:px-8" x-data="pengelolaanForm('{{ $pengelolaan->type }}', {{ json_encode($pengelolaan->extra_data ?? null) }}, {{ json_encode($pengelolaan->images ?? []) }})">
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-800">{{ __('cms.pengelolaan.edit_title') }}</h1>
        </div>
    </div>

    <form action="{{ route('cms.features.pengelolaan.pages.update', [$feature, $pengelolaan]) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <select name="type" x-model="type" required
                                ...
                                <option value="penyusutan" {{ $pengelolaan->type === 'penyusutan' ? 'selected' : '' }}>{{ __('cms.pengelolaan.type_penyusutan') }}</option>
                                <option value="penyimpanan" {{ $pengelolaan->type === 'penyimpanan' ? 'selected' : '' }}>{{ __('cms.pengelolaan.type_penyimpanan') }}</option>
                                <option value="preservasi" {{ $pengelolaan->type === 'preservasi' ? 'selected' : '' }}>{{ __('cms.pengelolaan.type_preservasi') }}</option>
                                <option value="pengolahan" {{ $pengelolaan->type === 'pengolahan' ? 'selected' : '' }}>{{ __('cms.pengelolaan.type_pengolahan') }}</option>
                                <option value="pemanfaatan" {{ $pengelolaan->type === 'pemanfaatan' ? 'selected' : '' }}>{{ __('cms.pengelolaan.type_pemanfaatan') }}</option>
                                <option value="penjangkauan" {{ $pengelolaan->type === 'penjangkauan' ? 'selected' : '' }}>{{ __('cms.pengelolaan.type_penjangkauan') }}</option>
                                <option value="akuisisi" {{ $pengelolaan->type === 'akuisisi' ? 'selected' : '' }}>{{ __('cms.pengelolaan.type_akuisisi') }}</option>
                            </select>
                        </div>
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="title" value="{{ $pengelolaan->title }}" required placeholder="{{ __('cms.pengelolaan.placeholder_title') }}"
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="rte-wrapper">
                        <div id="div_editor1">{!! $pengelolaan->description !!}</div>
                    </div>
                    ...
                    <div class="mb-4 p-3 bg-blue-50 border border-blue-100 rounded-lg flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <a href="{{ asset('storage/' . $pengelolaan->extra_data['file']) }}" target="_blank" class="text-xs text-blue-600 hover:underline">{{ basename($pengelolaan->extra_data['file']) }}</a>
                            </div>
                        </div>
                        <div class="flex items-center gap-2">
                            <input type="hidden" name="extra_data[file]" value="{{ $pengelolaan->extra_data['file'] }}">
                    ...
                    <div class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="extra_data[prinsip_title]" x-model="prinsip_title" class="w-full px-3 py-1.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" placeholder="{{ __('cms.pengelolaan.placeholder_prinsip_title') }}">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-700 mb-1">{{ __('cms.pengelolaan.label_prinsip_desc') }}</label>
                                <textarea name="extra_data[prinsip_desc]" x-model="prinsip_desc" rows="2" class="w-full px-3 py-1.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" placeholder="{{ __('cms.pengelolaan.placeholder_prinsip_desc') }}"></textarea>
                            </div>
                        </div>
                    ...
                    <div class="pt-6 border-t border-gray-100 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="mb-4 pt-3 border-t border-gray-100">
                                <label class="block text-xs font-semibold text-gray-600 mb-2">{{ __('cms.pengelolaan.label_upload_fasilitas') }}</label>
                                @php
                                ...
                                @if(count($existingFasilitas) > 0)
                                    <div class="mb-3 bg-gray-50 p-3 rounded-lg border border-gray-200">
                                        <label class="block text-[11px] font-semibold text-gray-500 mb-2">{{ __('cms.pengelolaan.label_current_images_del') }}</label>
                                        <div class="grid grid-cols-2 gap-3">
                                            @foreach($existingFasilitas as $img)
                                                <div class="flex items-center gap-2 bg-white p-1.5 rounded border border-gray-200 shadow-sm">
                                                    <img src="{{ asset('storage/' . $img) }}" class="h-12 w-12 object-cover rounded border border-gray-100">
                                                    <div class="flex-1 min-w-0">
                                                        <input type="hidden" name="extra_data[existing_fasilitas_images][]" value="{{ $img }}">
                                                        <label class="inline-flex items-center gap-1.5 text-[11px] text-red-600 hover:text-red-700 cursor-pointer font-medium">
                                                            <input type="checkbox" name="remove_fasilitas_images[]" value="{{ $img }}" class="rounded border-gray-300 text-red-600 focus:ring-red-500 w-3 h-3">
                                                            {{ __('cms.pengelolaan.btn_delete_check') }}
                                                        </label>
                                                    </div>
                                                </div>
                                            @endforeach
                        ...
                        <div>
                            ...
                            <div class="mb-4 pt-3 border-t border-gray-100">
                                <label class="block text-xs font-semibold text-gray-600 mb-2">{{ __('cms.pengelolaan.label_upload_ruang') }}</label>
                                @php
                                ...
                                @if(count($existingRuang) > 0)
                                    <div class="mb-3 bg-gray-50 p-3 rounded-lg border border-gray-200">
                                        <label class="block text-[11px] font-semibold text-gray-500 mb-2">{{ __('cms.pengelolaan.label_current_images_del') }}</label>
                                        <div class="grid grid-cols-2 gap-3">
                                            @foreach($existingRuang as $img)
                                                <div class="flex items-center gap-2 bg-white p-1.5 rounded border border-gray-200 shadow-sm">
                                                    <img src="{{ asset('storage/' . $img) }}" class="h-12 w-12 object-cover rounded border border-gray-100">
                                                    <div class="flex-1 min-w-0">
                                                        <input type="hidden" name="extra_data[existing_ruang_images][]" value="{{ $img }}">
                                                        <label class="inline-flex items-center gap-1.5 text-[11px] text-red-600 hover:text-red-700 cursor-pointer font-medium">
                                                            <input type="checkbox" name="remove_ruang_images[]" value="{{ $img }}" class="rounded border-gray-300 text-red-600 focus:ring-red-500 w-3 h-3">
                                                            {{ __('cms.pengelolaan.btn_delete_check') }}
                                                        </label>
                                                    </div>
                                                </div>
...
<script src="{{ asset('js/cms/features/pengelolaan/edit.js') }}?v={{ time() }}"></script>
@endpush
