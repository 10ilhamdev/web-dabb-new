...
@section('content')
<div class="space-y-6" x-data="pengelolaanManager()">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-800">{{ __('cms.pengelolaan.title', ['name' => $feature->name]) }}</h1>
            <p class="text-sm text-gray-500 mt-0.5">{{ __('cms.pengelolaan.desc') }}</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
        <div class="px-6 py-5 border-b border-gray-100 flex items-start justify-between">
            <div>
                <h2 class="text-base font-semibold text-gray-800">{{ __('cms.pengelolaan.list_title') }}</h2>
                <p class="text-sm text-gray-500 mt-0.5">{{ __('cms.pengelolaan.list_desc') }}</p>
            </div>
            <a href="{{ route('cms.features.pengelolaan.pages.create', $feature) }}"
                ...
                {{ __('cms.pengelolaan.add_button') }}
            </a>
        </div>

        <div>
            <table id="tablePengelolaanPages" class="w-full text-sm text-left">
                <thead class="bg-gray-50 border-b border-gray-100">
                    <tr>
                        <th class="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wide w-12">{{ __('cms.pengelolaan.col_no') }}</th>
                        <th class="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wide">{{ __('cms.pengelolaan.col_title') }}</th>
                        <th class="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wide">{{ __('cms.pengelolaan.col_type') }}</th>
                        <th class="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wide">{{ __('cms.pengelolaan.col_date') }}</th>
                        <th class="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wide text-center">{{ __('cms.pengelolaan.col_order') }}</th>
                        <th class="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wide text-center">Status</th>
                        <th class="px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wide text-center">{{ __('cms.pengelolaan.col_action') }}</th>
                    </tr>
                </thead>
                    ...
                    @forelse($pages as $index => $page)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                @php
                                    $typeLabels = [
                                        'penyusutan' => __('cms.pengelolaan.type_penyusutan'),
                                        'penyimpanan' => __('cms.pengelolaan.type_penyimpanan'),
                                        'preservasi' => __('cms.pengelolaan.type_preservasi'),
                                        'pengolahan' => __('cms.pengelolaan.type_pengolahan'),
                                        'pemanfaatan' => __('cms.pengelolaan.type_pemanfaatan'),
                                        'penjangkauan' => __('cms.pengelolaan.type_penjangkauan'),
                                        'akuisisi' => __('cms.pengelolaan.type_akuisisi'),
                                    ];
                                @endphp
                            ...
                            <td class="px-6 py-4 text-gray-600">
                             ...
                             <td class="px-6 py-4 text-center">
                                 <form action="{{ route('cms.features.pengelolaan.pages.toggle_visibility', [$feature, $page]) }}" method="POST" class="inline">
                                     @csrf
                                     @method('PATCH')
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('cms.features.pengelolaan.pages.edit', [$feature, $page]) }}"
                        ...
                        <tr>
                             <td colspan="7" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="text-gray-400 text-sm">{{ __('cms.pengelolaan.empty') }}</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
function pengelolaanManager() {
    return {
        ...
        submitDelete() {
            ...
            form.method = 'POST';
            form.action = `/cms/features/{{ $feature->id }}/pengelolaan/${this.deleteModal.id}`;
...
$(document).ready(function() {
    ...
    $('#tablePengelolaanPages').DataTable({
